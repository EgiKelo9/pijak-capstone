...
const Pricing = () => {
  return (
    <div className="mx-auto max-w-7xl px-4 md:px-8 w-full pt-8 md:pt-12">
  ...
  return (
    <div
      className={cn(
        ...
        "relative flex w-full flex-col flex-1 rounded-[2.5rem] p-3 gap-3 border-[0.4px] border-[#5D5D5D] bg-foreground",
        {
    ...
    >
      ...
      {plan.isRecommended && (
        <BorderBeam 
          duration={8} 
          size={200} 
          className="from-[#90FDF2] via-[#2BBAEE] to-[#90FDF2]" // Matching button colors
        />
      )}

      {/* Top Block: Header / Description */}
      {/* We keep relative here just to ensure children (like icons) stack correctly over gradients */}
      <div className="relative z-10 flex flex-col items-start p-8 md:p-10 rounded-3xl bg-gradient-to-r from-[#272727] to-[#333333] border-[0.4px] border-[#5D5D5D] grow">
        <plan.icon className="size-12 mb-4" />
        <div className="flex flex-col items-start text-left w-full gap-1">
          <h3 className="font-bold text-4xl leading-[1.1] text-[#F3F3F3]">
            {plan.name}
          </h3>
          <p className="text-xl md:text-2xl font-light text-[#F3F3F3]/40 leading-snug mt-1">
            {plan.description}
          </p>
        </div>
      </div>
      ...
      <div className="relative z-10 flex flex-col shrink-0 p-8 md:p-10 rounded-3xl bg-gradient-to-r from-[#272727] to-[#333333] border-[0.4px] border-[#5D5D5D]">
        {/* ... rest of your content (price, button, features) ... */}
        <div className="mb-2 flex flex-col items-start text-left w-full gap-0">
          <h4 className="font-bold text-5xl md:text-6xl text-[#F3F3F3] leading-[1.1]">
            {plan.price}
          </h4>
          <p className="text-xl font-light text-[#F3F3F3]/40">
            {plan.priceSuffix}
          </p>
        </div>
        ...
        <button
          className={cn(
            "my-8 w-full py-4 text-2xl font-medium transition-all rounded-2xl flex items-center justify-center",
            plan.isRecommended
        ...
        <ul className="mt-4 space-y-5">
          {plan.features.map((feature) => (
            <li className="flex items-center gap-4" key={feature}>
              ...
              />
              <span className="text-xl md:text-2xl font-light text-[#F3F3F3]">
                {feature}
              </span>
            </li>
          ))}
