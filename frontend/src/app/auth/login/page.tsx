...
import { JSX, SVGProps, useState, SubmitEvent } from "react";
...
export default function LoginPage() {
  ...
  const handleSubmit = async (event: SubmitEvent<HTMLFormElement>) => {
    ...
    try {
      await isLogin(email, password, remember);
    } catch (err) {
      ...
      <div className="flex w-full max-w-md flex-col gap-6 rounded-3xl border border-neutral-800/10 bg-white p-8 md:p-10 shadow-sm">
        ...
        <div className="flex flex-col items-center gap-3 text-center">
          ...
          <div className="flex flex-col gap-1 items-center">
            <h1 className="text-2xl md:text-3xl font-semibold tracking-tight text-neutral-800">
              Masuk ke <span className="text-[#2BBAEE]">Beez</span>
            </h1>
        ...
        {/* Form Content */}
        <form onSubmit={handleSubmit} className="space-y-5 mt-2" suppressHydrationWarning>
          ...
          <div className="space-y-1.5">
            <div className="flex items-center justify-between">
              ...
              <Link href="#" className="text-sm text-[#2BBAEE] font-medium hover:underline">
                Lupa Sandi?
              </Link>
            </div>
          ...
          {/* <div className="flex items-center space-x-2">
            <Checkbox id="remember" name="remember" value="on" defaultChecked className="data-[state=checked]:bg-[#2BBAEE] data-[state=checked]:border-[#2BBAEE]" />
          ...
          <div className="flex w-full flex-col gap-3 pt-2">
            <button
              type="submit"
              disabled={loading}
              className="w-full flex items-center justify-center gap-2 rounded-xl border border-neutral-800/10 bg-gradient-to-b from-[#90FDF2] to-[#2BBAEE] px-6 py-3 text-sm font-medium text-neutral-800 transition-all hover:opacity-90 disabled:opacity-50 active:scale-95 shadow-sm"
            >
              {loading ? "Masuk..." : "Masuk"}
            </button>
          </div>
