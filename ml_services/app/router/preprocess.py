...
from app.schemas.gemini import GeminiPreprocessRequest, GeminiPreprocessResponse
...
@router.post(
    "/run", 
    response_model=GeminiPreprocessResponse | Any,
    responses={
...
async def generate_preprocess(request: GeminiPreprocessRequest):
    ...
    Args:
        request (GeminiPreprocessRequest): Request body yang berisi dataset id, serta pendekatan preprocessing yang diinginkan (Cluster/Forecast/Both).
